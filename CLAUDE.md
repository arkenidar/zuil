...
- **Built-in bitmap text (landed 2026-06-15; bold/italic 2026-06-18)**: `zuil_set_font_scale`,
  `zuil_draw_text`, `zuil_set_font_style` (+ `ZUIL_FONT_REGULAR/BOLD/ITALIC`), and measurement
  `zuil_text_width` / `zuil_text_height` / `zuil_font_advance` — **replaces SDL3_ttf** (deps stay
  empty). Glyphs are baked **style pages** (regular/bold/italic/bold-italic), each a DejaVu Sans
  Mono face rendered 1-bit into a per-page **RLE container** (`src/font_atlas.{h,bin}`, generated
  by the dev-only, unshipped `tools/gen_font_atlas.py` via Pillow — only the bitmaps are committed).
  Cell size is **data-driven** (read from the container, not hardcoded). Both impls; see
  `docs/architecture.md` §10 (2026-06-15, 2026-06-18). Rich text stays a user-space styled-runs
  pattern. **Still ahead:** UTF-8 / Unicode coverage, then colored glyphs (emoji = a future RGBA
  page `depth`; RLE is for the 1-bit case only).
...
- **SDL3 dev libs**: `libsdl3-dev` (pkg-config name `sdl3`). Text is a built-in
  bitmap font, so the **runtime no longer needs `libsdl3-ttf-dev`**. The dev-only,
  unshipped `tools/gen_font_atlas.py` rasterizes TTF faces with **Pillow**
  (`python3-pil`) + the DejaVu Sans Mono fonts — needed only to *regenerate* the
  baked `src/font_atlas.{h,bin}`, never to build or run ZUIL.
