...
- **Built-in bitmap text (landed 2026-06-15)**: `zuil_set_font_scale`, `zuil_draw_text`, and
  measurement `zuil_text_width` / `zuil_text_height` / `zuil_font_advance` — a public-domain 8×8
  ASCII font baked into the binary (`src/font8x8.{h,bin}`, generated by the dev-only, unshipped
  `tools/gen_font8x8.py`), drawn as one tinted cropped+scaled atlas. **This replaces SDL3_ttf as
  the text mechanism** (deps stay empty). Both impls; see `docs/architecture.md` §10 (2026-06-15).
  Rich text is a user-space styled-runs pattern; bold/italic are a spec'd-not-built forward path
  (RLE style pages in a future big atlas + `zuil_set_font_style`).
...
- **SDL3 dev libs**: `libsdl3-dev` (pkg-config name `sdl3`). Text is a built-in
  bitmap font (2026-06-15), so the **runtime no longer needs `libsdl3-ttf-dev`**;
  it's at most an optional dep of the dev-only `tools/gen_font8x8.py` when baking
  the future big high-detail atlas.
