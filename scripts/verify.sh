...
#   5. Hedge — rebuild the .so with plain gcc (no Zig) and re-run the FFI smoke,
#      proving the toolchain hedge still stands. (skipped by --quick)
#
# Flags:
#   --quick        skip step 5 (the gcc-hedge re-verify) — for the hot loop / hook
...
step "1/5 lockstep (builds C + Zig impls, diffs against header)"
...
step "2/5 FFI smoke (LuaJIT) — expect linked SDL3 3.2.10"
expect "ZUIL ok" luajit examples/smoke.lua

step "3/5 ctypes smoke (Python)"
expect "ZUIL ok" python3 examples/smoke.py

step "4/5 M1 headless smoke (window + pump + draw, 30 frames)"
...
step "5/5 toolchain hedge — rebuild .so with plain gcc (no Zig), re-run FFI smoke"
mkdir -p zig-out/lib
...
echo; echo "verify: OK — lockstep + 3 smokes + gcc hedge all green (impl=$IMPL)"
